...
- [x] PR Size line mode renders completed labels at indexes `0`, `floor((count - 1) / 2)`, and `count - 1` when completed history exceeds 8 points; always renders the detached label when present; preserves 8-or-fewer plain-line and duration-comparison behavior; and retains every point in the screen-reader list.
