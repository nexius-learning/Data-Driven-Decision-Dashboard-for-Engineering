...
Status: Implemented as FEAT-003; later chart presentation updated by FIX-002 and FIX-004.

> Current follow-up note: this brief defines the Phase 03 PR Size baseline. FIX-002 added detached current-week confidence behavior, and FIX-004 later changed the completed-week chart history to the shared 16 completed-week comparison presentation. The current UI does not render the originally listed Largest PR table column.
...
## Goal
The dashboard shows median PR size (lines changed), trend context, team breakdown, and flags teams with oversized PR patterns — so leads can detect and address large-PR habits before they compound into slower cycles.
...
4. Below, the trend chart shows median PR size over time, plus a detached current-week-so-far point when current-week measured PRs exist.
5. A team breakdown table shows each team's median size and trend direction without author names.
...
- Median size trend chart. The original Phase 03 baseline was 8 weeks; FIX-004 later expanded completed history to 16 completed weeks.
- Team breakdown table: Team, PRs merged, Median size, Trend (↑ if current 8-week median exceeds prior 8-week median by >10%; ↓ if prior 8-week median exceeds current 8-week median by >10%; → otherwise; shown as '—' if fewer than 3 PRs in either the current or prior period)
- Trend confidence semantics: current-week-so-far notes include measured PR count and "This value may change"; `< 3` measured PRs are explicitly labeled low sample.
...
- **Trend chart**: Original Phase 03 showed median PR size per week for the last 8 weeks; FIX-004 later expanded completed history to 16 completed weeks. Weeks with 0 PRs show no data point (gap, not zero). The trend chart shows data points where PRs with size data exist, regardless of whether the metric card is in "baseline pending" state.
- **Team breakdown table**: Shows all teams with ≥ 1 PR with non-null size data in the selected window. Current UI columns are Team, PRs merged (count with size data), Median size (lines), and Trend (↑/↓/→ based on comparison to prior 8-week median; "—" if fewer than 3 PRs in either the current or prior period). Sorted by median size descending.
